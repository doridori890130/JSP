...
public class CommentController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		String pathUri = uri.substring("/cmt/".length()); //modify/1
		String path = pathUri;
		String pathVar = "";
		log.info(path);
		if(pathUri.contains("/")) {
			path= pathUri.substring(0, pathUri.lastIndexOf("/")); //modify
			pathVar = pathUri.substring(pathUri.lastIndexOf("/")+1);//1, 10 숫자만 추출
		}
		log.info(path);
		log.info(pathVar);
		...
		case "remove":
			try {
				...
				int cno = Integer.parseInt(request.getParameter("cnoVal")); //쿼리스트링
				log.info(">>>cno : "+cno);
