...
async function postCommentToServer (cmtData){
	try{
		...
		const result = await resp.text();// isOk - controller 에서 보내는 결과
		return result;
	}catch(error){
		console.log(error);
	}
...
async function getCommentListFromServer(bno){
	...
	const cmtList = await resp.json(); // 댓글 객체가 리턴 // 가져온 변수에대한 형변환
	return cmtList;
...
function spreadCommentList(result){ // 댓글의 List
	console.log(result);
...
async function updateCommetFromServer(cnoVal, cmtText1, writer){
	try{
...
document.addEventListener('click',(e)=>{	// e 내가 클릭한 객체
	if(e.target.classList.contains('cmtModBtn')){
		...
		//비동기 통신
		updateCommetFromServer(cnoVal,cmtText1,writer).then(result=>{
			if(result>0){
				alert("댓글 수정 완료!");
				console.log(result);
